...
export default {
  data() {
    ...
    return {
      ...
      user: {
        //定义用户对象  主要封装传递的数据 更改用户状态 修改或者添加用户信息
        uid: 0,
        uname: "",
        password: "",
        images: "",
  ...
  methods: {
    changeStats(row) {
      ...
      this.$http
        .post("http://localhost/user/changeStats", this.user)
        .then((res) => {
    ...
    update() {
      this.$refs["user"].validate((valid) => {
        if (valid) {
          this.$http
            .post("http://localhost/user/update", this.user)
            .then((res) => {
    ...
    save() {
      this.$refs["user"].validate((valid) => {
        if (valid) {
          this.$http
            .post("http://localhost/user/save", this.user)
            .then((res) => {
              if (res.data.code == 200) {
                this.$message({
                  showClose: true,
    ...
    remove(scope) {
      this.$http
        .post("http://localhost/user/remove?uid=" + scope.row.uid)
        .then((res) => {
          if (res.data.code == 200) {
            this.$message({
              message: res.data.message,
    ...
    findPager() {
      this.$http
        .get("http://localhost/user/findPager", {
          params: {
